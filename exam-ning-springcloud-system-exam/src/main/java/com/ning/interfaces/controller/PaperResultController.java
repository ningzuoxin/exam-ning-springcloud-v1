...
public class PaperResultController {
    ...
    @GetMapping(value = "/{id}")
    public PaperResultDetailDTO detail(@PathVariable(value = "id") @ApiParam(name = "id", example = "1") Long id) {
        return paperResultAppService.getResultDetail(id);
    }

    //    @PreAuthorize("@ss.hasPermi('exam:result:marks')")
    @ApiOperation(value = "查询待批阅试卷列表")
    @GetMapping(value = "/to-grading")
    public PageWrapper<PaperResultDTO> toGrading(@RequestParam(value = "pageNum", defaultValue = "1") @ApiParam(name = "pageNum", example = "1") Integer pageNum,
                                                 @RequestParam(value = "pageSize", defaultValue = "10") @ApiParam(name = "pageSize", example = "10") Integer pageSize) {
        return paperResultAppService.toGrading(pageNum, pageSize);
    }

    //    @PreAuthorize("@ss.hasPermi('exam:result:doMark')")
    @ApiOperation(value = "评分")
    @PostMapping(value = "/grading")
    public PaperResultDTO grading(@RequestBody PaperResultGradingRequest request) {
        ...
        return paperResultAppService.grading(paperResultId, questionGradingList);
    }
