...
public class PaperController {
    ...
    public List<PaperTypeDTO> types() {
        return paperAppService.types();
    }

    @ApiOperation(value = "添加试卷")
    @PostMapping(value = "")
    public PaperDTO add(@RequestBody @Valid AddPaperRequest request) {
        PaperDTO paperDTO = paperAssembler.toDTO(request);
        return paperAppService.add(paperDTO);
    }

    //    @PreAuthorize("@ss.hasPermi('exam:testPaper:page')")
    @ApiOperation(value = "分页查询试卷列表")
    @GetMapping(value = "/page")
    public PageWrapper<PaperDTO> page(@RequestParam(value = "type", required = false) @ApiParam(name = "type", example = "") Integer type,
        ...
        return paperAppService.page(type, keyword, pageNum, pageSize);
    }

    //    @PreAuthorize("@ss.hasPermi('exam:testPaper:preview')")
    @ApiOperation(value = "试卷预览")
    @GetMapping(value = "/{id}/preview")
    public PaperPreviewDTO preview(@PathVariable(value = "id") @ApiParam(name = "id", example = "1") Long id) {
        return paperAppService.preview(id);
    }
    ...
    public Boolean submit(@PathVariable(value = "id") Long id, @RequestBody @Valid PaperSubmitRequest request) {
        ...
        return paperAppService.submit(id, request.getTimeUsed(), paperQuestionSubmitDTOList);
    }

    @ApiOperation(value = "发布试卷")
    @PostMapping(value = "/{id}/publish")
    public PaperDTO publish(@PathVariable(value = "id") @ApiParam(name = "id", example = "1") Long id) {
        return paperAppService.publish(id);
    }

    @ApiOperation(value = "删除试卷")
    @DeleteMapping(value = "/{id}")
    public Boolean delete(@RequestParam(value = "id") @ApiParam(name = "id", example = "1") Long id) {
        return paperAppService.delete(id);
    }
